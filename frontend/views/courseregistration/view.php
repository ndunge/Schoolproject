...
<div class="pure-u-1"></div>
	...
	<tr>
		...
		<td width="50%">
			<label><b>Customer Balance <span style="color:#F00"></span></b></label>
			<div data-role="select">
             

			<input type="text" name="student-balance" value="300,000" disabled>   
...
<form id="data_form" method="POST" action="view?id=<?= $model['Reg_ Transacton ID']; ?>">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="3">
		...
		foreach ($Selected AS $key => $row)
		{
			...
			<tr>		
				<td>
					<input class="units-selector" data-amount="<?= $s_amount;  ?>" name="<?= "CK_".$s_id; ?>" type="checkbox" value="1" <?= $Checked; ?> />
				</td>
				<td>
					<?= $s_name; ?>
				</td>	
	...
	<h1>Optional Services</h1>
		...
		foreach ($result2  AS $key => $raw)
		{
			$s_id = $raw["Code"];
			$s_name = $raw["Description"];
			$Checked = '';
			?>
			<tr>		
				<td>
...
<script type="text/javascript">
	$(function(){
		$('.units-selector').off('click').on('click', function(evt){
			//get the unit' cost
			var cost = $(this).data('amount');
			 
			//get the original balance
			var balance = $('input[name="student-balance"]').val();
			//var value = parseFloat("554,20".replace(",", "."));
			//check if selected
			if($(this).is(':checked')){
				//reduce the balance
//alert(parseFloat(balance.replace(",", "")));
				balance = (parseFloat(balance.replace(",", "")) - parseFloat(cost.replace(",", "")));
			...
			//var value2 = number_format(balance,2);
			
			//alert(value2);
			$('input[name="student-balance"]').val(balance.toFixed(2));
		});
	})
</script>
<div class="pure-u-1"></div>