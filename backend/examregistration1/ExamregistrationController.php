...
class ExamregistrationController extends Controller
{
	...
	*/
	public function actionView()
	{
		...
		// print_r($params); exit;
		if (!empty($params)) {
			$_exam_types = explode(",", $params['ExamTypes']);
			
			foreach ($_exam_types as $key => $_exam) {
				$marks = Examregistration::find()->where([
					'Programme ID'=>$params['ProgrammeID'],
					'Term ID' => $params['TermID'],
					'Stage ID' => $params['StageID'],
					'Academic Year' => $params['AcademicYear'],
					'Exam Type ID' => $_exam,
					'ProgrammeCourseID' => $params['ProgrammeCourseID'],
					'Student ID' => $params['StudentID'],
				])->one();
				$marks['Actual Mark'] = $params[$_exam];
				$marks->save();
			}
			//print_r( $_exam_types ); exit;
		}
		...
		// 	// 	}
		// 	// 	// $model = Examregistration::find()->asArray()->where("[Programme ID] = '".$gparams['ProgrammeID']."' AND [Term ID] = '".$gparams['TermID']. "' AND [Stage ID] = '".$gparams['StageID']."' AND [Academic Year]='".$gparams['AcademicYear']."'" )->one();
		// 	// 	return $this->render('view', [ 'model' => $model]);
		// } else
		// 	{
		// 		return $this->render('view', [ 'model' => $model]);
